...
\begin{table}[H]
    ...
    \begin{tabular}{lc}
    \toprule
        \multicolumn{2}{c}{Accuracy Score} \\ \midrule
        evaluations & adult \\  % & dorothea  \\
        \midrule \midrule
        % optimistic oracle &  1.000 \\ %   &  0.989 \\
        % overfitted oracle & 0.838 \\ %  &  0.969 \\
        \midrule
        $A^*$ (training) &  0.839 $(\pm 0.069)$ \\ %  & 0.973 \\
        $E^*$ (training) &  {\bf 0.860 $(\pm 0.060)$} \\ %  & {\bf 1.000} \\
        \cmidrule(lr){1-2}
        delta (training) & \cellcolor{green!25} 0.021 \\ %  & \cellcolor{green!25} 0.027  \\
        \midrule \midrule
        $A^*$ (testing) & 0.823 $(\pm 0.019)$ \\ %  & 0.937 \\ 
        $E^*$ (testing) & {\bf 0.831 $(\pm 0.013)$} \\ %  & {\bf 0.946} \\
        \cmidrule(lr){1-2}
        delta (testing) & \cellcolor{green!25} 0.008 \\ %  & \cellcolor{green!25} 0.009  \\
        \midrule
    \bottomrule
    \end{tabular}
    ...
    \caption{Evaluation in the Adult dataset in terms of ....
    Maximum number of classifiers set to 50.
    The base model selection strategy was set to \emph{double-fault}.
    No detriment constraints in use.
    }
...
\begin{table}[H]
    ...
    \begin{tabular}{lc}
    \toprule
        \multicolumn{2}{c}{Precisión} \\ \midrule
        Método & Resultados en \emph{Adult} \\
        \midrule \midrule
        FERM & $0.164 \pm 0.010$ \\
        ...
        FairBO (LL) & $0.175 \pm 0.007$ \\  \midrule
        BFair (20) & $0.170$ {\tiny \bf SP=}$0.087$ \\
    \bottomrule
    \end{tabular}

    \caption{Restriccion $DSP \leq 0.1 $.}
    \label{table:comparison-adult-dsp01}
\end{table}
